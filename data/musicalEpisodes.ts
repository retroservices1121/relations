...
export const musicalEpisodes: MusicalEpisode[] = [
  {
    ...
    scenes: [
      ...
      {
        duration: 6,
        prompt: "EXACTLY ONE Joe and ONE Danda only. ANATOMY LOCK FOR DANDA: Danda has exactly TWO arms total for the entire shot, no more and no less. Her RIGHT arm is the ONLY arm holding the shoe, from the first frame through the final frame. Her LEFT arm stays bent at the elbow beside her torso in a nervous guarded pose and NEVER touches the shoe. Do not create, duplicate, ghost, blend, fade in, fade out, or morph any extra arm, hand, forearm, wrist, elbow, or limb at any time. Never show two hands or two arms holding the shoe. Keep both shoulders and both arms continuously attached and anatomically consistent through every frame. Danda reluctantly approaches the tiny spider with the shoe held only in her RIGHT hand. Her knees bend cautiously while her LEFT arm remains bent beside her body. Joe stays comfortably several steps behind her, totally relaxed, silently pointing toward the spider and giving an encouraging thumbs-up like a coach. Joe never approaches to kill it and never looks scared. The spider moves only a tiny distance. Danda immediately jumps backward in alarm, STILL holding the shoe only in her RIGHT hand while her LEFT arm remains bent. Joe remains perfectly calm and simply points to where the spider moved. Both mouths closed and still. No screams, gasps, speech, lip sync, vocal sounds or generated music.",
