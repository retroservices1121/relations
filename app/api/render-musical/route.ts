...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (durations.some((duration) => !Number.isFinite(duration) || duration <= 0 || duration > 30)) return NextResponse.json({ error: "Each musical scene timestamp must be greater than 0 and no longer than 30 seconds." }, { status: 400 });

    workDir = await fs.mkdtemp(path.join(os.tmpdir(), "relations-musical-"));
    const musicPath = path.join(workDir, "song.wav"); await downloadFile(musicUrl, musicPath);
    ...
    for (let index = 0; index < sceneUrls.length; index += 1) {
      ...
      const exactDuration = durations[index];
      const holdSeconds = Math.max(3, Math.ceil(exactDuration));
      await execFileAsync(ffmpegPath, ["-y","-i",inputPath,"-vf",`scale=720:1280:force_original_aspect_ratio=increase,crop=720:1280,fps=30,format=yuv420p,tpad=stop_mode=clone:stop_duration=${holdSeconds}`,"-t",exactDuration.toFixed(3),"-an","-c:v","libx264","-profile:v","high","-level","4.0","-preset","veryfast","-crf","20","-r","30","-movflags","+faststart",outputPath]);
    ...
    return NextResponse.json({ url: stored.url, key: stored.key, musical: true, songFirst: true, songDuration, sceneAudioDiscarded: true, householdTheme: false, longSceneHoldSupported: true });
