...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (durations.some((duration) => !Number.isFinite(duration) || duration <= 0 || duration > 15)) return NextResponse.json({ error: "Each musical scene timestamp must be greater than 0 and no longer than 15 seconds." }, { status: 400 });
