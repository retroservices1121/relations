...
export async function POST(request: Request) {
  try {
    ...
    return NextResponse.json({ url: stored.url, key: stored.key, requestId: result.requestId, duration: data.duration || duration, seed: data.seed, model: "minimax/music-3" });
  } catch (error) {
