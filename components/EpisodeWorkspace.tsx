...
export default function EpisodeWorkspace({ episode }: { episode: Episode }) {
  ...
  const [sceneTimes, setSceneTimes] = useState<Record<number, number>>({});
  ...
  async function loadBalance() {
    try {
      ...
      setBalance(typeof data.balance === "number" ? data.balance : null); setBalanceError("");
    } catch (error) { setBalanceError(error instanceof Error ? error.message : "Balance unavailable"); }
  }

  useEffect(() => {
    let cancelled = false;
    setJoeUrl(localStorage.getItem(JOE_STORAGE_KEY) || ""); setDandaUrl(localStorage.getItem(DANDA_STORAGE_KEY) || "");
    const defaultOverlays = Object.fromEntries(episode.scenes.map((_, index) => [index, defaultOverlay(index)]));
    ...
    void (async () => {
      try {
        const response = await fetch(`/api/project?episodeId=${encodeURIComponent(episode.id)}`, { cache: "no-store" }); const data = await response.json();
        if (!response.ok) throw new Error(data.error || "Could not load Railway project data"); if (cancelled) return;
        const dbStates: Record<number, SceneState> = {}; const dbOverlays: Record<number, OverlayConfig> = { ...defaultOverlays };
        for (const row of (data.scenes || []) as DatabaseScene[]) {
          const index = Number(row.scene_index); if (!Number.isInteger(index) || index < 0 || index >= episode.scenes.length) continue;
          ...
          const defaults = defaultOverlays[index];
          dbOverlays[index] = { text: row.overlay_text ?? defaults.text, position: ["top","middle","bottom"].includes(row.overlay_position || "") ? row.overlay_position as OverlayPosition : defaults.position, start: Number(row.overlay_start ?? defaults.start), end: Number(row.overlay_end ?? defaults.end) };
        }
        setSceneStates((prev) => ({ ...prev, ...dbStates })); setOverlays((prev) => ({ ...prev, ...dbOverlays })); setFinalUrl(data.finalUrl || ""); setStorageError("");
      } catch (error) { if (!cancelled) setStorageError(`${error instanceof Error ? error.message : "Railway project storage unavailable"} Local device cache is being used until Postgres is connected.`); }
      finally { if (!cancelled) setProjectLoaded(true); }
    })();
    void loadBalance(); return () => { cancelled = true; };
  }, [episode.id, episode.scenes, projectStorageKey]);

  useEffect(() => { if (projectLoaded) localStorage.setItem(projectStorageKey, JSON.stringify({ sceneStates, overlays, finalUrl })); }, [sceneStates, overlays, finalUrl, projectLoaded, projectStorageKey]);

  function persistReference(character: CharacterKey, url: string) {
    if (character === "joe") { setJoeUrl(url); if (url) localStorage.setItem(JOE_STORAGE_KEY, url); else localStorage.removeItem(JOE_STORAGE_KEY); }
    else { setDandaUrl(url); if (url) localStorage.setItem(DANDA_STORAGE_KEY, url); else localStorage.removeItem(DANDA_STORAGE_KEY); }
  }

  async function uploadReference(character: CharacterKey, file?: File) {
    if (!file) return; setUploading(character); setUploadError("");
    try { const formData = new FormData(); formData.append("file", file); const response = await fetch("/api/upload-reference", { method: "POST", body: formData }); const data = await response.json(); if (!response.ok) throw new Error(data.error || "Upload failed"); if (!data.url) throw new Error("Upload completed without a file URL"); persistReference(character, data.url); }
    catch (error) { setUploadError(error instanceof Error ? error.message : "Upload failed"); } finally { setUploading(null); }
  }
  ...
  async function saveGeneratedVideo(index: number, requestId: string, sourceUrl: string) {
    ...
    const response = await fetch("/api/persist-video", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ sourceUrl, episodeId: episode.id, sceneIndex: index, requestId }) }); const data = await response.json();
    if (!response.ok) { setStorageError(data.error || "Permanent storage is not configured."); setSceneStates((prev) => ({ ...prev, [index]: { status: "done", videoUrl: sourceUrl, requestId, persisted: false } })); return; }
    setStorageError(""); setSceneStates((prev) => ({ ...prev, [index]: { status: "done", videoUrl: data.url, requestId, persisted: true } }));
  }

  async function pollForResult(index: number, requestId: string, selectedModel: string) {
    for (let attempt = 0; attempt < 180; attempt += 1) { await sleep(3000); const response = await fetch(`/api/generate-video?requestId=${encodeURIComponent(requestId)}&model=${encodeURIComponent(selectedModel)}`, { cache: "no-store" }); const data = await response.json(); if (!response.ok) throw new Error(data.error || "Could not check generation status"); if (data.status === "COMPLETED" && data.videoUrl) { await saveGeneratedVideo(index, requestId, data.videoUrl); void loadBalance(); return; } setSceneStates((prev) => ({ ...prev, [index]: { ...prev[index], status: "generating", requestId } })); }
    throw new Error("Generation is still running. Try Generate Scene again in a moment to start a new job.");
  }

  function clearSavedFinal() { setFinalUrl(""); setFinalError(""); setFinalActionError(""); void fetch("/api/project", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "clear-final", episodeId: episode.id }) }).catch(() => undefined); }

  async function generateScene(index: number) {
    const scene = episode.scenes[index]; const imageUrls = [joeUrl, dandaUrl].map((v) => v.trim()).filter(Boolean); const selectedModel = model;
    if (imageUrls.length < 2) { setSceneStates((prev) => ({ ...prev, [index]: { status: "error", error: "Upload both approved cartoon references for Joe and Danda before generating this scene." } })); return; }
    clearSavedFinal(); setSceneStates((prev) => ({ ...prev, [index]: { status: "queued" } }));
    try {
      const response = await fetch("/api/generate-video", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ model: selectedModel, duration: scene.duration, imageUrls, prompt: `Use the approved recurring cartoon character assets exactly as shown in the references. @Image1 is Joe. @Image2 is Danda. Preserve their faces, hairstyles, clothing identity and overall 2D cartoon design. CHARACTER PROPORTIONS ARE LOCKED: Joe has an average, slightly stocky everyday-dad build. Danda is only moderately shorter than Joe, like a normal adult couple with a modest height difference. Both are normally proportioned adults. STYLE IS LOCKED: simple flat 2D cartoon comedy with clean bold outlines, exaggerated facial expressions, physical reactions, playful visual timing and readable uncluttered backgrounds. Use visual pantomime and held facial poses. All soundtrack and text overlays are added later in Studio. Vertical 9:16 relationship-comedy short. Scene action: ${scene.prompt}` }) });
      const data = await response.json(); if (!response.ok) throw new Error(data.error || "Generation failed"); if (!data.requestId) throw new Error("fal did not return a request ID"); setSceneStates((prev) => ({ ...prev, [index]: { status: "generating", requestId: data.requestId } })); await pollForResult(index, data.requestId, selectedModel);
    } catch (error) { setSceneStates((prev) => ({ ...prev, [index]: { status: "error", error: error instanceof Error ? error.message : "Generation failed" } })); void loadBalance(); }
  }
  ...
  async function shareFinalVideo() {
    if (!finalUrl) return; setSharingFinal(true); setFinalActionError("");
    try { const response = await fetch(finalDownloadUrl(), { cache: "no-store" }); if (!response.ok) { const data = await response.json().catch(() => ({})); throw new Error(data.error || "Could not prepare the video for sharing."); } const blob = await response.blob(); const file = new File([blob], `${episode.id}.mp4`, { type: "video/mp4" }); const shareData = { files: [file], title: episode.title, text: episode.title }; if (!navigator.share || (navigator.canShare && !navigator.canShare(shareData))) throw new Error("Native video sharing is not supported in this browser. Use Download MP4, then save the video to Photos or Gallery."); await navigator.share(shareData); }
    catch (error) { if (!(error instanceof DOMException && error.name === "AbortError")) setFinalActionError(error instanceof Error ? error.message : "Could not share the final video."); } finally { setSharingFinal(false); }
  }

  return <div className="workspace">
    <div className="workspaceTop"><div><a className="backLink" href="/">← Episode Library</a><span className="eyebrow">JOE + DANDA</span><h1>{episode.title}</h1><p>{episode.hook}</p></div><div className="workspaceControls"><div className="creditBalance" onClick={() => void loadBalance()} title="Tap to refresh fal balance"><span>fal credits</span><b>{balance === null ? (balanceError ? "Unavailable" : "Loading…") : `$${balance.toFixed(2)}`}</b></div><label>Model<select value={model} onChange={(e) => setModel(e.target.value)}><option value="seedance-fast">Seedance 2 Fast</option><option value="seedance-standard">Seedance 2 Standard</option></select></label></div></div>
    ...
    <section className="referencePanel"><div><span className="eyebrow">LOCKED CHARACTER LIBRARY</span><h2>Joe + Danda references</h2><p>Use the final cartoon character images here. Once uploaded, they are remembered and reused automatically across every episode on this device.</p><p className="statusText">Silent-cartoon format is locked: Studio adds soundtrack and precisely timed overlays after generation.</p><p className="statusText">Production storage: Railway Postgres saves project data and Cloudflare R2 saves video files.</p>{uploadError && <p className="errorText">{uploadError}</p>}{storageError && <p className="errorText">{storageError}</p>}</div>
      <div className="referenceInputs"><div className="characterRef"><label>Joe cartoon reference {joeUrl && "✓ Locked"}</label>{joeUrl && <img className="referenceThumb" src={joeUrl} alt="Joe cartoon reference" />}<label className="uploadButton">{uploading === "joe" ? "Uploading Joe…" : joeUrl ? "Replace Joe Cartoon" : "Upload Joe Cartoon"}<input type="file" accept="image/jpeg,image/png,image/webp" disabled={uploading !== null} onChange={(e) => uploadReference("joe", e.target.files?.[0])} /></label><input value={joeUrl} onChange={(e) => persistReference("joe", e.target.value)} placeholder="Or paste the approved Joe cartoon URL" /></div>
    ...
    <div className="sceneList">{episode.scenes.map((scene, index) => {
      ...
      return <article className="sceneCard" key={index}><div className="sceneMeta"><span>SCENE {index + 1}</span><b>{scene.duration}s</b></div><h3>{scene.prompt}</h3>
        ...
        <div className="overlayEditor"><span className="eyebrow">TEXT OVERLAY — TIMED IN STUDIO</span><label>Overlay text<textarea value={overlay.text} onChange={(e) => updateOverlay(index, { text: e.target.value })} placeholder="Optional caption or [start-end] timed captions" /></label><div className="overlayGrid"><label>Position<select value={overlay.position} onChange={(e) => updateOverlay(index, { position: e.target.value as OverlayPosition })}><option value="top">Top</option><option value="middle">Middle</option><option value="bottom">Bottom</option></select></label><label>Start (sec)<input type="number" min="0" max={scene.duration} step="0.1" value={overlay.start} onChange={(e) => updateOverlay(index, { start: Number(e.target.value) })} /></label><label>End (sec)<input type="number" min="0" max={scene.duration} step="0.1" value={overlay.end} onChange={(e) => updateOverlay(index, { end: Number(e.target.value) })} /></label></div>{timed.length > 1 && <p className="statusText">Timed lines preview individually. Bracket timing is never shown in the video.</p>}</div>
