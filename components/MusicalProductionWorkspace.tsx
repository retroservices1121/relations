...
export default function MusicalProductionWorkspace({ episode }: { episode: MusicalEpisode }) {
    ...
    return {
      ...
      scenes: episode.scenes.map((scene, index) => {
        ...
        return {
          ...
          prompt: `${scene.prompt} MUSICAL TIMING TARGET: this shot will occupy exactly ${exact.toFixed(2)} seconds in the final song-driven edit, from song timestamp ${timings[index].start.toFixed(2)}s to ${timings[index].end.toFixed(2)}s. Make the main visual action happen early enough to read clearly within that interval, then hold the final reaction pose through the end so Studio can trim the clip precisely to the music.`,
        };
      }),
    };
  }, [episode, timings]);
  ...
  return <>
    ...
    {timedEpisode ? <>
      <div style={{margin:"0 0 16px",padding:"12px 16px",border:"1px solid #999",borderRadius:12,background:"#f7f7f7"}}>
        <b>STEP 2 — Generate scenes to the locked song timeline</b>
        <p style={{margin:"6px 0 0"}}>The scene lengths below are derived from the song timestamps. Generate the scenes only after you are happy with the song and boundaries above.</p>
      </div>
