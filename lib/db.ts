...
function pool() {
  ...
  if (!global.relationsPool) {
    global.relationsPool = new Pool({ connectionString: process.env.DATABASE_URL });
  }
...
export async function ensureSchema() {
  if (!global.relationsSchemaReady) {
    global.relationsSchemaReady = (async () => {
      ...
      `);
      await db.query(`
        CREATE TABLE IF NOT EXISTS relations_scenes (
          ...
          overlay_start DOUBLE PRECISION NOT NULL DEFAULT 0,
          overlay_end DOUBLE PRECISION NOT NULL DEFAULT 0,
          updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
          PRIMARY KEY (episode_id, scene_index)
        );
      `);
    })();
  }
...
export async function saveSceneVideo(input: {
  ...
  await ensureSchema();
  await pool().query(
    `INSERT INTO relations_scenes (episode_id, scene_index, video_url, request_id, persisted, updated_at)
     VALUES ($1, $2, $3, $4, TRUE, NOW())
     ON CONFLICT (episode_id, scene_index) DO UPDATE SET
       video_url = EXCLUDED.video_url,
       request_id = EXCLUDED.request_id,
       persisted = TRUE,
       updated_at = NOW()`,
    [input.episodeId, input.sceneIndex, input.videoUrl, input.requestId],
  );
}
  ...
  await pool().query(
    `INSERT INTO relations_scenes (episode_id, scene_index, overlay_text, overlay_position, overlay_start, overlay_end, updated_at)
     ...
     ON CONFLICT (episode_id, scene_index) DO UPDATE SET
       ...
       overlay_start = EXCLUDED.overlay_start,
       overlay_end = EXCLUDED.overlay_end,
       updated_at = NOW()`,
...
export async function saveFinalVideo(episodeId: string, finalUrl: string) {
  await ensureSchema();
  await pool().query(
    `INSERT INTO relations_projects (episode_id, final_url, updated_at)
     VALUES ($1, $2, NOW())
     ON CONFLICT (episode_id) DO UPDATE SET final_url = EXCLUDED.final_url, updated_at = NOW()`,
    [episodeId, finalUrl],
  );
}
...
export async function clearFinalVideo(episodeId: string) {
  ...
  await pool().query(
    `INSERT INTO relations_projects (episode_id, final_url, updated_at)
     VALUES ($1, NULL, NOW())
     ON CONFLICT (episode_id) DO UPDATE SET final_url = NULL, updated_at = NOW()`,
    [episodeId],
  );
}
...
export async function loadProject(episodeId: string) {
  ...
  const [project, scenes] = await Promise.all([
    ...
    pool().query(
      `SELECT scene_index, video_url, request_id, persisted, overlay_text, overlay_position, overlay_start, overlay_end
       FROM relations_scenes WHERE episode_id = $1 ORDER BY scene_index`,
      [episodeId],
    ),
  ]);
  return {
    finalUrl: project.rows[0]?.final_url || "",
    scenes: scenes.rows,
  };
}
